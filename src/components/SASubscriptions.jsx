...
export default function SASubscriptions() {
  ...
  const confirmRenewal = async () => {
    ...
    try {
      ...
      // 2. Record Payment - ✅ Use 'method' not 'payment_method'
      try {
        const { error: paymentError } = await supabase
          ...
          .insert({
            ...
            status: 'Confirmed',
            method: 'Manual', // ✅ Correct column name
            date: today.toISOString()
          });

        if (paymentError) {
          console.error('⚠️ Payment Insert Failed:', paymentError);
          toast.warning('⚠️ Renewal successful but payment log failed.');
        } else {
          toast.success('✅ Subscription renewed & payment logged!');
        }
      } catch (payErr) {
        console.error('❌ Payment Insert Exception:', payErr);
      }
